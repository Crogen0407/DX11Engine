...
WPARAM Core::Run(CoreDesc& desc)
{
	...
	TIMEMANAGER->Init(_desc.hWnd);
	INPUTMANAGER->Init(_desc.hWnd);
...
LRESULT CALLBACK Core::WndProc(HWND handle, UINT message, WPARAM wParam, LPARAM lParam)
{
	switch (message)
	{
	case WM_SIZE:
